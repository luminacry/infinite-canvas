...
export default function PromptsPage() {
    ...
    return (
        <div className="bg-background flex h-full flex-col overflow-hidden text-stone-800 dark:text-stone-100">
            <main
                className="bg-background min-h-0 flex-1 overflow-y-auto bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] px-6 py-8 [background-size:16px_16px] dark:bg-[radial-gradient(rgba(245,245,244,.16)_1px,transparent_1px)]"
                onScroll={handleListScroll}
            >
                <div className="pb-8">
                    <div className="mx-auto max-w-5xl text-center">
                        <h1 className="text-4xl font-semibold tracking-tight">提示词中心</h1>
                        <p className="text-muted-foreground mt-3 text-sm">共 {totalPrompts} 条提示词，按标题、标签与分类快速查找灵感。</p>
                    </div>
                    {query.isLoading ? (
                        ...
                        <>
                            <div className="mx-auto mt-8 w-full max-w-2xl">
                                <div className="relative">
                                    ...
                                    <Input className="w-full pl-9" value={titleKeyword} placeholder="按标题查询" onChange={(e) => setTitleKeyword(e.target.value)} />
                                </div>
                            </div>
                ...
                {!query.isLoading ? (
                    <div>
                        <div className="mx-auto grid max-w-7xl gap-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                            {promptItems.map((item) => (
                                <PromptCard
                                    ...
                                    extraAction={
                                        <Button variant="outline" size="sm" onClick={() => savePromptAsset(item)}>
                        ...
                        {promptItems.length === 0 ? <div className="text-muted-foreground py-16 text-center text-sm">没有找到匹配的提示词</div> : null}
                        <div className="text-muted-foreground mx-auto mt-6 max-w-7xl text-center text-xs">
